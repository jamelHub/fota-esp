  ...
  case State::CHECK_WIFI:
    if (WiFi.status() == WL_CONNECTED)
    {
      ...
      if (currentMillis - lastUpdateTime >= updateInterval)
      {
        ...
        String updateURL = (String(serverAddress) + "/updateStatus?name=" + urlEncode(hostName)); // Use the urlEncode() function here
...
#elif defined(ESP32)
          t_httpUpdate_return ret = ESP32shttpUpdate.update(String(serverAddress) + "/upload?name=" + urlEncode(hostName), firmwareVersion); // Use urlEncode() function here
#endif
          switch (ret)
          {
...
void ESPOTADASH::sendFirmwareFlashingInitiated()
{
  ...
  String firmwareInitiatedURL = String(serverAddress) + "/firmwareInitiated?name=" + urlEncode(hostName); // Include the hostname as a query parameter
...
String ESPOTADASH::getCommandFromServer()
{
  ...
  String commandURL = String(serverAddress) + "/getCommand?name=" + urlEncode(hostName);
